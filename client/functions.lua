...
function KGCore.Functions.CreateClientCallback(name, cb)
    KGCore.ClientCallbacks[name] = cb
end

function KGCore.Functions.TriggerClientCallback(name, cb, ...)
    if not KGCore.ClientCallbacks[name] then return end
    KGCore.ClientCallbacks[name](cb, ...)
end

function KGCore.Functions.TriggerCallback(name, cb, ...)
    KGCore.ServerCallbacks[name] = cb
    TriggerServerEvent('KGCore:Server:TriggerCallback', name, ...)
end

function KGCore.Debug(resource, obj, depth)
    TriggerServerEvent('KGCore:DebugSomething', resource, obj, depth)
end

-- Player

function KGCore.Functions.GetPlayerData(cb)
    if not cb then return KGCore.PlayerData end
    cb(KGCore.PlayerData)
end
...
function KGCore.Functions.HasItem(items, amount)
    return exports['kg-inventory']:HasItem(items, amount)
end
...
function KGCore.Functions.LookAtEntity(entity, timeout, speed)
...
function KGCore.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
...
function KGCore.Functions.IsWearingGloves()
    local ped = PlayerPedId()
    local armIndex = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped)
    if model == `mp_m_freemode_01` then
        if KGCore.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if KGCore.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function KGCore.Functions.Notify(text, texttype, length, icon)
    local message = {
        action = 'notify',
...
function KGCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('progressbar') ~= 'started' then error('progressbar needs to be started in order for KGCore.Functions.Progressbar to work') end
...
function KGCore.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function KGCore.Functions.GetObjects()
...
function KGCore.Functions.GetPlayersFromCoords(coords, distance)
...
function KGCore.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = KGCore.Functions.GetPlayersFromCoords(coords)
...
function KGCore.Functions.GetPeds(ignoreList)
...
function KGCore.Functions.GetClosestPed(coords, ignoreList)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    ignoreList = ignoreList or {}
    local peds = KGCore.Functions.GetPeds(ignoreList)
    local closestDistance = -1
    local closestPed = -1
    for i = 1, #peds, 1 do
...
function KGCore.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function KGCore.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function KGCore.Functions.LoadModel(model)
...
function KGCore.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
    ...
    KGCore.Functions.LoadModel(model)
...
function KGCore.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function KGCore.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return KGCore.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function KGCore.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function KGCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            bodyHealth = KGCore.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = KGCore.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = KGCore.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = KGCore.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = KGCore.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = KGCore.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
function KGCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function KGCore.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function KGCore.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function KGCore.Functions.RequestAnimDict(animDict)
...
function KGCore.Functions.GetClosestBone(entity, list)
...
function KGCore.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function KGCore.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and joaat(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    KGCore.Functions.LoadModel(modelHash)
...
function KGCore.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function KGCore.Functions.LoadAnimSet(animSet)
...
function KGCore.Functions.LoadParticleDictionary(dictionary)
...
function KGCore.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    KGCore.Functions.LoadParticleDictionary(dict)
...
function KGCore.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    KGCore.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function KGCore.Functions.GetStreetNametAtCoords(coords)
...
function KGCore.Functions.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function KGCore.Functions.GetCardinalDirection(entity)
...
function KGCore.Functions.GetCurrentTime()
    local obj = {}
...
function KGCore.Functions.GetGroundZCoord(coords)
...
function KGCore.Functions.GetGroundHash(entity)
    local coords = GetEntityCoords(entity)
